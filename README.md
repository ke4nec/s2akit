...
- **托盘常驻**：关窗即隐藏到托盘（非退出）；左键托盘图标显示/隐藏主窗口；右键菜单：
  ...
  - 账号行右键：测试该账号（自动选模型）/ 选择模型测试…（自动或从该账号模型列表指定）/ 禁用该账号
...
- 「账号」页选择目标分组（会记忆），点「测试全部」后按首 token 排序，点行内「启用」完成切换。
- 默认测试模型的选择规则（未显式指定时，候选均须在该账号的模型列表中）：
  ① 该账号**上次测试实际使用的模型**（持久化记忆，跨重启生效）；
  ② 平台默认：OpenAI 账号用 `astra` 系列，Claude（anthropic/antigravity）账号优先 `opus-5`、次选 `opus`；
  ③ 列表第一个非媒体文本模型。
  也可在账号行「测试」菜单里手动从模型列表选择；在设置里固定模型可强制统一口径
  （注意 sub2api 的 `/accounts/:id/models` 响应顺序不稳定，但模型集合稳定，上述规则均基于集合匹配）。
- 调试 WebView（可选）：
