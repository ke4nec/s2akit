...
- **主窗口账号页**：顶栏分组胶囊切换（会记忆）；表格支持列排序与分页，行首状态竖条 +
  ...
  无边框自绘标题栏（空白拖拽、最小化/最大化、关闭隐藏到托盘）。
- **主窗口 API Key 页**：与账号页同套表格风格（排序/分页/行右键模型子菜单）；用量列显示
  当日费用与请求/token 量，悬停看明细（请求次数、输入/输出/缓存 token、缓存率、
  平均 token/次与费用）；行右键“查看额度”可指定默认 Key（再点恢复汇总全部），
  托盘菜单顶部用量随之切换
...
- **托盘常驻**：关窗即隐藏到托盘（非退出）；左键托盘图标显示/隐藏主窗口；右键菜单：
  - 顶部单行两区：左侧是**组 | key 内容切换**，
    右侧显示当天用量——费用与 token 总量（M），默认汇总 All Keys，可在某 Key 的子菜单点“查看额度”只看它，
    悬停看请求次数与输入/输出/缓存明细（`/admin/usage/stats?period=today`，按服务器时区取自然日），
    点击强制刷新一次即时值（转圈保底 1 秒 + 到值闪光反馈）；两区独立响应事件，互不影响
  - 组模式：首行为当前分组（二级菜单切换分组），余下为该组账号，行悬停（100ms 意图延迟）
    或点击，在主菜单旁另起**独立子菜单窗口**（主菜单宽度不变，右侧放不下自动翻到左侧）：
    选择模型测试…（模型名过长悬停看全名）/ 启用禁用
  - key 模式：API Key 列表（仅启用/禁用两种状态），行悬停或点击同样级联子菜单窗口：
    先调 `/v1/models` 取该 key 支持的模型列表再测试，用法与账号一致
  - 刷新账号列表 / 显示主窗口 / 退出
